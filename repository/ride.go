...
func CreateRide(tx *sqlx.Tx, ride *models.Ride) error {
	...
	return tx.QueryRowx(query, ride.PassengerID, ride.PickupLat, ride.PickupLng, ride.PickupAddress,
		ride.DropLat, ride.DropLng, ride.DropAddress).
		Scan(&ride.ID, &ride.Status, &ride.RequestedAt)
}

func GetNearbyDrivers(tx *sqlx.Tx, lat, lng float64, radiusMeters int) ([]string, error) {
	...
	err := tx.Select(&driverIDs, query, lng, lat, radiusMeters)
